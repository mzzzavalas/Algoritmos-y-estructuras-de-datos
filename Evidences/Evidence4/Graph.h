...
private:
    ...
    int findShortest(vector<bool> &status, vector<int> &cost) const;
public:
    Graph();
    ...
    void bfs(T vertex);
    void dfs(T vertex);
    pair<vector<int>, vector<int>> dijkstra(T vertex) const;
...
template <class T>
int Graph<T>::findVertex(T vertex) const {
...
void Graph<T>::bfs(T vertex) {
    // Crear un objeto de flujo de archivo
    ofstream out("output608-3.out");

    // Guardar el buffer de salida estándar
    streambuf* coutbuf = cout.rdbuf();

    // Redirigir cout al archivo de salida
    ...
    if (vertexIndex != -1) {
      ...
      // Restaurar el flujo original de cout
    cout.rdbuf(coutbuf);
...
template <class T>
int Graph<T>::findShortest(vector<bool> &status, vector<int> &cost) const{
...
template <class T>
pair<vector<int>, vector<int>> Graph<T>::dijkstra(T vertex) const{
    // Validamos que el vértice de inicio exista
    int vertexIndex = findVertex(vertex);
    if (vertexIndex != -1) {
        // Creamos la lista de estado
        vector<bool> status(vertices.size(), false);
        // Creamos la lista de distancia
        vector<int> distance(vertices.size(), INT_MAX);
        // Creamos la lista de tiempo
        vector<int> time(vertices.size(), INT_MAX);
        // Creamos la lista de paths
        vector<int> path(vertices.size(), -1);
        // Actualizamos en 0 la distancia y el tiempo del vertexIndex (vértice inicial)
        distance[vertexIndex] = 0;
        time[vertexIndex] = 0;
        // Creamos un índice para almacenar el índice del vértice con menor distancia
        ...
        while (shortestIndex != -1) {
            // Actualizamos el estado del vértice con la distancia menor
            status[shortestIndex] = true;
            // Iteramos los vecinos del vértice con la distancia menor
            for (auto neighbor : adjacencyList[shortestIndex]) {
                // Buscamos el índice del vecino
                int neighborIndex = findVertex(neighbor.target);
                // Validamos si el estado del vecino es falso
                if (!status[neighborIndex]) {
                    // Comparamos si la distancia del vecino es mayor a la distancia del vértice con menor distancia + la distancia
                    if (distance[neighborIndex] > distance[shortestIndex] + neighbor.DistanceT) {
                        // Actualizamos la distancia del vecino con la distancia del vértice con menor distancia + la distancia
                        distance[neighborIndex] = distance[shortestIndex] + neighbor.DistanceT;
                        // Actualizamos el tiempo del vecino con el tiempo del vértice con menor tiempo + el tiempo
                        time[neighborIndex] = time[shortestIndex] + neighbor.TimeT;
                        // Actualizamos el path del vértice vécino con el indice del vértice con menor distancia
                        path[neighborIndex] = shortestIndex;
                    }
                }
            }
            // Buscamos el vértice con la menor distancia y estado en falso
            shortestIndex = findShortest(status, distance);
        }
...
return {vector<int>(distance), vector<int>(time)};
}
...
void Graph<T>::print() {
    ...
    for (int i = 0; i < vertices.size(); ++i) {
        ...
        for (int j = 0; j < adjacencyList[i].size(); ++j) {
            cout << adjacencyList[i][j].target << " " << adjacencyList[i][j].TimeT << " "
                 << adjacencyList[i][j].DistanceT << " " << adjacencyList[i][j].TimeC << " "
                 << adjacencyList[i][j].DistanceC << " |  ";
        }
        cout << endl;
    }
